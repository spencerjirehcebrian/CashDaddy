...
const errorHandler = (err, _req, res, _next) => {
    ...
    if (err.name === 'CastError' && 'kind' in err && err.kind === 'ObjectId') {
        const customError = new error_types_1.InvalidObjectIdError(`Invalid ObjectId format: A param is not a valid`);
        const serializedError = customError.serializeErrors();
        return (0, response_1.sendResponse)(res, serializedError.statusCode, false, 'Invalid ObjectId', serializedError);
    }
